...
class PF_3DOF_DR(MCLocalization):
    ...
    def GetInput(self):
        ...
        xsk = self.robot.fs(self.robot.xsk_1, usk) # Simulate the robot motion
        ...
        uk=np.array([uk_x, uk_y, delta_theta]).T
        uk = Pose3D(uk)

        self.robot.xsk_1 = xsk

        #mandar otro QK
        sigma = 0.1 
        Qk = np.diag([sigma**2 , sigma**2])
    ...
    def MotionModel(self, particle, u, noise):
        ...
        particle = Pose3D(particle).oplus(u+noise)
...
if __name__ == '__main__':
    ...
    particles_np = np.random.multivariate_normal(x0.reshape(3,) , P0 ,size=(3,n_particles)) 
    print(particles_np[:,1])
    particles = []
    for i in range(particles_np.shape[1]):
        print(particles_np[:,i].reshape(3,1))
